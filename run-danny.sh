...
function run_remotely () {
  ...
  ssh -n $HOST \
    "cd $RUN_DIR && DANNY_ESTIMATOR_SAMPLES=$DANNY_ESTIMATOR_SAMPLES DANNY_SKETCH_EPSILON=$DANNY_SKETCH_EPSILON DANNY_BASELINES_PATH=$DANNY_BASELINES_PATH DANNY_SEED=$DANNY_SEED RUST_LOG=$RUST_LOG DANNY_PROCESS_ID=$ID DANNY_HOSTS=$DANNY_HOSTS DANNY_THREADS=$DANNY_THREADS $DANNY_EXEC $ARGS"
}

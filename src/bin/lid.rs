    ...
    F: Fn(&D, &D) -> f64 +Send + Sync+ 'static,
{
    ...
    output_path.set_extension(format!("lid"));
    ...
    data.par_iter()
        .for_each(|(c, v)| {
            ...
            for (i, range) in ranges.iter().enumerate() {
                let lid = lids[i] / counts[i] as f64;
                let lid = -1.0 / lid;
                if !(lid.is_nan() || lid.is_infinite()) {
                    send.send((c.clone(), range.clone(), lid)).expect("Error in sending");
                } 
            }
        });
        drop(send);
...
fn main() {
    ...
    let ranges: Vec<f64> = matches .value_of("RANGE")
        .unwrap()
        .split(",")

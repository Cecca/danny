...
pub fn load_global_vecs_new<D>(
    ...
    Arc<RwLock<Arc<ChunkedDataset<u32, D>>>>,
    Arc<RwLock<Arc<ChunkedDataset<u32, D>>>>,
    Arc<Mutex<Sender<(u8, u8)>>>,
    Arc<Barrier>,
    std::thread::JoinHandle<()>,
...
{
    // These two maps hold the vectors that need to be accessed by all threads in this machine.
    let global_left_write: Arc<RwLock<Arc<ChunkedDataset<u32, D>>>> =
        Arc::new(RwLock::new(Arc::new(ChunkedDataset::empty())));
    let global_right_write: Arc<RwLock<Arc<ChunkedDataset<u32, D>>>> =
        Arc::new(RwLock::new(Arc::new(ChunkedDataset::empty())));
    let global_left_read = global_left_write.clone();
    let global_right_read = global_right_write.clone();
    ...
    let reader_handle = thread::spawn(move || {
        ...
        let mut global_left = global_left_write.write().unwrap();
        *global_left = Arc::new(left_builder.finish());
        let mut global_right = global_right_write.write().unwrap();
        *global_right = Arc::new(right_builder.finish());
